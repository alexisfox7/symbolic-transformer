...
"""
Simplified script to train a Pure Symbolic Transformer.
All gradient accumulation complexity has been removed for clarity.

Usage examples:
python examples/train_symbolic_example.py --preset small --batch_size 32
python examples/train_symbolic_example.py --use_proj --use_v --batch_size 16 --num_epochs 5
"""
...
from config.config import SymbolicConfig, get_preset_config, print_config
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Train Pure Symbolic Transformer')
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./outputs/symbolic_simple')
...
def main():
    ...
    print("="*60)
    print("SIMPLIFIED SYMBOLIC TRANSFORMER TRAINING")
    print("="*60)
    ...
    if args.use_reconstruction:  
        ...
        logger.info("Using Symbolic Transformer WITH reconstruction loss")
    elif args.use_standard_norm:
        model = get_model("SymbolicStandardNorm", config=config).to(device)
        logger.info("Using Symbolic Transformer WITH standard LayerNorm")
    else:
    ...
    # Save model
    model_path = os.path.join(args.output_dir, "symbolic_model.pt")
    torch.save({
