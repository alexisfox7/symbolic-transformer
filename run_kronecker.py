...
def plot_matrix_heatmaps(layer_data, matrix_key, title, save_path):
    ...
    for i, layer_data_item in enumerate(layer_data):
        ...
        # Plot heatmap - determine colormap range for centering at 0
...
def plot_head_interactions(kronecker_data, output_dir):
    ...
    if best_v_layer:
        ...
        im1 = axes[0].imshow(v_matrix, cmap='RdBu_r', vmin=-abs(v_matrix).max(), vmax=abs(v_matrix).max())
    ...
    if best_proj_layer:
        ...
        im2 = axes[1].imshow(proj_matrix, cmap='RdBu_r', vmin=-abs(proj_matrix).max(), vmax=abs(proj_matrix).max())
