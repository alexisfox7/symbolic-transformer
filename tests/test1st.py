...
def decompose_with_first_layer(model, input_ids, layer_idx, position_idx, k=3):
    ...
    with torch.no_grad():
        ...
        # MAGNITUDE-CORRECTED VERSION: Normalize to unit vectors
        # Normalize target residual
        residual_norm = torch.norm(residual)
        residual_normalized = residual / residual_norm if residual_norm > 0 else residual
        ...
        recon_raw_norm = (A_raw_norm @ coeffs_raw_norm.unsqueeze(1)).squeeze()
        recon_trans_norm = (A_trans_norm @ coeffs_trans_norm.unsqueeze(1)).squeeze()
        
        # Scale back to original magnitude
        recon_raw_scaled = recon_raw_norm * residual_norm
        recon_trans_scaled = recon_trans_norm * residual_norm
        
        # Compute magnitude-corrected errors
        error_raw_norm = torch.norm(residual - recon_raw_scaled).item()
        error_trans_norm = torch.norm(residual - recon_trans_scaled).item()
        
        # APPROACH #4: Direction-first magnitude-corrected least squares
        # Step 1: Solve for direction using normalized vectors
        residual_dir = residual / torch.norm(residual)
        raw_dirs = raw_embeddings / torch.norm(raw_embeddings, dim=1, keepdim=True)
        trans_dirs = transformed_embeddings / torch.norm(transformed_embeddings, dim=1, keepdim=True)
        
        # Solve for directional coefficients
        A_raw_dir = raw_dirs.T
        A_trans_dir = trans_dirs.T
        b_dir = residual_dir.unsqueeze(1)
        
        coeffs_raw_dir = torch.linalg.lstsq(A_raw_dir, b_dir).solution.squeeze()
        coeffs_trans_dir = torch.linalg.lstsq(A_trans_dir, b_dir).solution.squeeze()
        
        # Step 2: Find optimal magnitude scaling
        directional_recon_raw = (raw_dirs * coeffs_raw_dir.unsqueeze(1)).sum(dim=0)
        directional_recon_trans = (trans_dirs * coeffs_trans_dir.unsqueeze(1)).sum(dim=0)
        
        target_magnitude = torch.norm(residual)
        recon_magnitude_raw = torch.norm(directional_recon_raw)
        recon_magnitude_trans = torch.norm(directional_recon_trans)
        
        magnitude_scale_raw = target_magnitude / recon_magnitude_raw if recon_magnitude_raw > 0 else 1
        magnitude_scale_trans = target_magnitude / recon_magnitude_trans if recon_magnitude_trans > 0 else 1
        
        # Final reconstruction
        final_recon_raw = directional_recon_raw * magnitude_scale_raw
        final_recon_trans = directional_recon_trans * magnitude_scale_trans
        
        # Compute direction-first errors
        error_raw_dir = torch.norm(residual - final_recon_raw).item()
        error_trans_dir = torch.norm(residual - final_recon_trans).item()
        ...
        # Add magnitude-corrected results
        results['coeffs_raw_norm'] = coeffs_raw_norm
        results['coeffs_transformed_norm'] = coeffs_trans_norm
        results['reconstruction_raw_norm'] = recon_raw_scaled
        results['reconstruction_transformed_norm'] = recon_trans_scaled
        results['error_raw_norm'] = error_raw_norm
        results['error_transformed_norm'] = error_trans_norm
        
        # Add direction-first results
        results['coeffs_raw_dir'] = coeffs_raw_dir
        results['coeffs_transformed_dir'] = coeffs_trans_dir
        results['reconstruction_raw_dir'] = final_recon_raw
        results['reconstruction_transformed_dir'] = final_recon_trans
        results['error_raw_dir'] = error_raw_dir
        results['error_transformed_dir'] = error_trans_dir
        results['magnitude_scale_raw'] = magnitude_scale_raw
        results['magnitude_scale_trans'] = magnitude_scale_trans
...
def analyze_first_layer_decomposition(model, text, device='cuda'):
    ...
    for layer_idx in [0, 1, 3, 6, 9, 11]:
        ...
        for pos_idx in range(len(tokens)):
            ...
            # Magnitude-corrected metrics
            error_raw_norm_pct = 100 * results['error_raw_norm'] / residual_norm
            error_trans_norm_pct = 100 * results['error_transformed_norm'] / residual_norm
            improvement_norm = error_raw_norm_pct - error_trans_norm_pct
            
            # Direction-first metrics
            error_raw_dir_pct = 100 * results['error_raw_dir'] / residual_norm
            error_trans_dir_pct = 100 * results['error_transformed_dir'] / residual_norm
            improvement_dir = error_raw_dir_pct - error_trans_dir_pct
            ...
            print(f"    Original improvement: {improvement:.1f} percentage points")
            
            print(f"\n  === MAGNITUDE-CORRECTED (Unit Vector) ===")
            print(f"  Raw embeddings (normalized):")
            print(f"    Coefficients: {results['coeffs_raw_norm'].cpu().numpy()}")
            print(f"    Error: {results['error_raw_norm']:.3f} ({error_raw_norm_pct:.1f}%)")
            
            print(f"\n  First-layer transformed (normalized):")
            print(f"    Coefficients: {results['coeffs_transformed_norm'].cpu().numpy()}")
            print(f"    Error: {results['error_transformed_norm']:.3f} ({error_trans_norm_pct:.1f}%)")
            print(f"    Unit vector improvement: {improvement_norm:.1f} percentage points")
            
            print(f"\n  === DIRECTION-FIRST MAGNITUDE-CORRECTED ===")
            print(f"  Raw embeddings (direction-first):")
            print(f"    Dir coefficients: {results['coeffs_raw_dir'].cpu().numpy()}")
            print(f"    Magnitude scale: {results['magnitude_scale_raw']:.3f}")
            print(f"    Error: {results['error_raw_dir']:.3f} ({error_raw_dir_pct:.1f}%)")
            
            print(f"\n  First-layer transformed (direction-first):")
            print(f"    Dir coefficients: {results['coeffs_transformed_dir'].cpu().numpy()}")
            print(f"    Magnitude scale: {results['magnitude_scale_trans']:.3f}")
            print(f"    Error: {results['error_transformed_dir']:.3f} ({error_trans_dir_pct:.1f}%)")
            print(f"    Direction-first improvement: {improvement_dir:.1f} percentage points")
            ...
            print(f"\n  Embedding norms:")
            print(f"    Raw: {[f'{n:.1f}' for n in raw_norms]}")
            print(f"    Transformed: {[f'{n:.1f}' for n in trans_norms]}")
