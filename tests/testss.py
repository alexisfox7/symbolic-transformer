...
def visualize_decomposed_key_attention(
    ...
    # Apply softmax only to decomposed components (exclude 'original' at index 0)
    # This converts raw attention scores to relative probabilities within each position
    # Each column (position) will sum to 1.0 across the decomposed components only
    attention_softmax = np.zeros_like(attention_matrix)
    for pos in range(n_positions):
        # Only apply softmax to decomposed components (skip original at index 0)
        decomp_scores = attention_matrix[1:, pos]  # Skip 'original' row
        exp_scores = np.exp(decomp_scores - decomp_scores.max())  # Numerical stability
        decomp_softmax = exp_scores / exp_scores.sum()  # Normalize to sum=1
        
        # Keep original score as-is, apply softmax to decomposed components
        attention_softmax[0, pos] = attention_matrix[0, pos]  # Keep original unchanged
        attention_softmax[1:, pos] = decomp_softmax  # Apply softmax to decomposed
    ...
    ax1.set_yticklabels(y_labels, fontsize=18)
    ax1.set_xticks(range(n_positions))
    ax1.set_xticklabels(tokens, rotation=45, ha='right', fontsize=16)
    ax1.set_title(f'Raw Attention Scores from Query "{tokens[query_position]}" to Decomposed Keys', fontsize=20, fontweight='bold')
    ax1.set_xlabel('Key Position', fontsize=18, fontweight='bold')
    ax1.set_ylabel('Representation Type', fontsize=18, fontweight='bold')
    ...
    for i in range(len(rep_types)):
        for j in range(n_positions):
            ...
            ax1.text(j, i, f'{val:.1f}', ha='center', va='center',
                    color='white' if val < 0 else 'black', fontsize=14, fontweight='bold')
    ...
    ax2.set_yticklabels(y_labels, fontsize=18)
    ax2.set_xticks(range(n_positions))
    ax2.set_xticklabels(tokens, rotation=45, ha='right', fontsize=16)
    ax2.set_title('Softmax Normalized Attention (Relative Within Each Position)', fontsize=20, fontweight='bold')
    ax2.set_xlabel('Key Position', fontsize=18, fontweight='bold')
    ax2.set_ylabel('Representation Type', fontsize=18, fontweight='bold')
    ...
    for i in range(len(rep_types)):
        for j in range(n_positions):
            ...
            ax2.text(j, i, f'{val:.2f}', ha='center', va='center',
                    color='white' if val > 0.5 else 'black', fontsize=14, fontweight='bold')
    ...
    # Additional plot: Stacked bar chart showing decomposition (exclude 'original')
    fig2, ax3 = plt.subplots(figsize=(16, 8))
    
    # Only include decomposed components (exclude 'original' which is at index 0)
    decomp_labels = y_labels[1:]  # ['word1', 'word2', 'word3', 'dark_matter']
    decomp_softmax = attention_softmax[1:, :]  # Skip first row (original)
    colors = ['lightgreen', 'peachpuff', 'lightcoral', 'lightgray']
    
    bottom = np.zeros(n_positions)
    
    for i, rep_type in enumerate(decomp_labels):
        ax3.bar(range(n_positions), decomp_softmax[i, :], 
               bottom=bottom, label=rep_type, color=colors[i], alpha=0.8)
        bottom += decomp_softmax[i, :]
    
    ax3.set_xticks(range(n_positions))
    ax3.set_xticklabels(tokens, rotation=45, ha='right', fontsize=16)
    ax3.set_ylabel('Attention Weight', fontsize=18, fontweight='bold')
    ax3.set_title(f'Stacked Attention Distribution from "{tokens[query_position]}" to Decomposed Keys', fontsize=20, fontweight='bold')
