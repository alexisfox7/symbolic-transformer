...
def run_multi_prompt_experiment(model, prompts, device='cuda', k=3):
    ...
    print(f"Running experiment on {len(prompts)} prompts (first position only for speed):")
...
def create_confidence_plot(results, save_path='poster_figure_with_confidence.png'):
    ...
    ax.text(0.02, 0.98, f'Average improvement (layers 1-11): {improvement:.1f} percentage points\n'
                        f'n = {len(results["prompts"])} prompts, first position only', 
...
def print_summary_results(results):
    ...
    print("\n" + "=" * 70)
    print("SUMMARY RESULTS FOR POSTER")
    print("=" * 70)
    
    print(f"Number of prompts: {len(results['prompts'])}")
    print(f"Analysis: First position only (for computational efficiency)")
    ...
    print(f"  Raw embeddings:           {avg_raw:.1f}% ± {np.mean(results['raw_sems'][1:]):.1f}")
    print(f"  First-layer transformed:  {avg_trans:.1f}% ± {np.mean(results['transformed_sems'][1:]):.1f}")
    ...
    # Show range of improvements
    layer_improvements = results['raw_means'][1:] - results['transformed_means'][1:]
...
def main():
    ...
    # Multiple prompts for confidence intervals (kept short for speed)
    prompts = [
        "The cat sat",
        "She was walking", 
        "John likes pizza",
