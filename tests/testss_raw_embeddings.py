...
def get_raw_embedding(model, token_id, position_idx, seq_length, device='cuda'):
    """
    Get raw embedding for a token at a specific position.
    
    Args:
        token_id: The token to get embedding for
        position_idx: Position where the token should be placed
        seq_length: Total sequence length (for positional encoding)
    
    Returns:
        The raw embedding (word + position) for that token
    """
    with torch.no_grad():
        # Get token embedding
        token_embedding = model.transformer.wte(torch.tensor([token_id], device=device))
        
        # Get positional embedding for this position
        position_embedding = model.transformer.wpe(torch.tensor([position_idx], device=device))
        
        # Combine token and position embeddings
        raw_embedding = token_embedding + position_embedding
        
        return raw_embedding.squeeze(0)
...
def get_residual_and_decompose(model, input_ids, layer_idx, position_idx):
    ...
    Now using raw embeddings (word + position) for decomposition.
    """
    ...
    # Use raw embeddings instead of first-layer transformed embeddings
    top3_embeddings = []
    for token_id in top3_indices:
        raw_embedding = get_raw_embedding(
            model, token_id.item(), position_idx, seq_length, device
        )
        top3_embeddings.append(raw_embedding)
...
def visualize_decomposed_key_attention(
    ...
    ax1.set_title(f'Raw Attention Scores from Query "{tokens[query_position]}" to Decomposed Keys (Raw Embeddings)')
    ...
    ax3.set_title(f'Stacked Attention Distribution from "{tokens[query_position]}" to Decomposed Keys (Raw Embeddings)')
